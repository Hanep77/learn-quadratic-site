      ...
      <nav className="max-w-screen-lg h-full m-auto flex justify-between items-center">
        ...
        <div className={`fixed md:static bg-[rgba(28,3,50,1)] min-h-screen md:bg-transparent z-50 inset-0 mt-16 pt-4 md:pt-0 md:mt-0 ${!sidebar && 'translate-x-full'} transition md:translate-x-0 flex`}>
          <ul className="md:flex md:gap-1 flex flex-col md:flex-row items-center gap-5 text-lg font-medium px-4 md:px-0 w-full">
            <li><Link href={"/home"} onClick={closeSidebar} className={`py-1 px-2 rounded-md ${pathname == "home" ? "text-violet-900 bg-white" : "hover:text-violet-900 hover:bg-white"}`}>Home</Link></li>
            <li><Link href={"/learn"} onClick={closeSidebar} className={`py-1 px-2 rounded-md ${pathname == "learn" ? "text-violet-900 bg-white" : "hover:text-violet-900 hover:bg-white"}`}>Learn</Link></li>
            <li><Link href={"/calculator"} onClick={closeSidebar} className={`py-1 px-2 rounded-md ${pathname == "calculator" ? "text-violet-900 bg-white" : "hover:text-violet-900 hover:bg-white"}`}>Calculator</Link></li>
            ...
            <li><Link href={"/team"} onClick={closeSidebar} className={`block md:hidden py-1 px-2 rounded-md ${pathname == "quiz" ? "text-violet-900 bg-white" : "hover:text-violet-900 hover:bg-white"}`}>Our Team</Link></li>
          </ul>
        </div>

        <div className="hidden md:block">
          <Link href={"/team"} className="px-5 py-1 border rounded-md bg-violet-100 text-violet-950">Our Team</Link>
        </div>
